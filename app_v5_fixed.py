...
# ── IMPROVEMENT #1: Resource limits for child processes ──────
import resource

# ── IMPROVEMENT #2: Safe math evaluator (replaces eval()) ────
import operator
import struct

# ── IMPROVEMENT #3: TOTP 2FA support ────────────────────────
import hmac
import struct as _struct
import time as _time_mod
import base64 as _b64

# ── IMPROVEMENT #4: Circuit breaker for Telegram API ────────
import functools
...
# ── IMPROVEMENT #6: Safe Math Evaluator (replaces eval()) ────
# Tokenizer + recursive-descent parser — supports +, -, *, /, //, %, **, ()
# No eval/exec/__import__ — mathematically safe.
_SAFE_OPS = {
...
def safe_eval(expr: str) -> float:
    """
    Evaluate a math expression safely — no eval(), no code execution.
    Supports: +, -, *, /, //, %, **, parentheses, unary minus.
    Raises ValueError on anything that isn't pure math.
    """
    # Quick guard: only allow digits, operators, parentheses, dots, spaces
    cleaned = expr.strip()
    if not cleaned:
...
app = Flask(__name__)
# ✅ IMPROVEMENT #5: Generate random secret key (never hardcoded)
app.secret_key = os.environ.get('DASHBOARD_SECRET', secrets.token_hex(32))
# Fix #12: No hardcoded default password — generate random if not set
DASHBOARD_PASSWORD = os.environ.get('DASHBOARD_PASS', '')
if not DASHBOARD_PASSWORD:
    DASHBOARD_PASSWORD = secrets.token_hex(16)
    logger.warning(f"⚠️ DASHBOARD_PASS not set! Generated random password: {DASHBOARD_PASSWORD}")
    logger.warning("⚠️ Set DASHBOARD_PASS env var to use your own password!")
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .navbar h1 { color:#7c3aed; font-size:1.4rem; display:flex; align-items:center; gap:10px; }
  .navbar .logo { width:36px; height:36px; border-radius:8px; object-fit:cover; }
...
{% if not logged_in %}
<div class="login-box">
  <img src="/logo" style="width:80px;height:80px;border-radius:16px;margin-bottom:12px;object-fit:cover;" alt="Logo">
  <h2>🖤 Dark Shadow</h2>
...
{% else %}
<div class="navbar">
  <h1><img src="/logo" class="logo" alt="Logo"> 🖤 Dark Shadow Dashboard</h1>
...
def dashboard():
    ...
    # Fix #9: don't close thread-local connection — get_db() reuses it
    for j in sj:
...
@app.route('/logo')
def serve_logo():
    """Serve the Dark Script logo for the dashboard."""
    if os.path.exists(DARK_SCRIPT_LOGO_PATH):
        return app.send_static_file(DARK_SCRIPT_LOGO_PATH) if os.path.dirname(DARK_SCRIPT_LOGO_PATH) == app.static_folder else \
               (open(DARK_SCRIPT_LOGO_PATH, 'rb').read(), 200, {'Content-Type': 'image/jpeg'})
    return '', 404

@app.route('/health')
def health():
    """✅ IMPROVEMENT #18: Real health check — verifies DB + Telegram connectivity."""
    checks = {}
    healthy = True
    ...
    try:
        ...
        checks['telegram'] = f'error: {str(e)[:50]}'
        healthy = False

    # Check disk space
    try:
        disk = psutil.disk_usage('/')
        checks['disk'] = f'{disk.percent}% used'
        if disk.percent > 95:
            healthy = False
            checks['disk'] += ' ⚠️ CRITICAL'
    except Exception:
        checks['disk'] = 'unknown'
...
def run_flask():
    ...
    # Fix #11: Use gunicorn in production if available, fallback to Flask dev server
    if os.environ.get('FLASK_ENV') == 'development':
        app.run(host='0.0.0.0', port=port, debug=True, use_reloader=False)
    else:
        # Production: Flask dev server (gunicorn started via Procfile)
...
# Fix #6: Define actual plans instead of empty dict
...
# ── IMPROVEMENT #7: Upload rate limiting ─────────────────────
UPLOAD_RATE_LIMIT       = 5        # max uploads per minute per user
UPLOAD_RATE_WINDOW      = 60       # seconds
upload_rate_data        = defaultdict(list)  # user_id -> [timestamps]

# ── IMPROVEMENT #8: Process resource limits (per-script) ────
MAX_SCRIPT_MEMORY_MB    = 512      # max RAM per script (MB)
MAX_SCRIPT_CPU_SECS     = 300      # max CPU seconds (5 min)
MAX_SCRIPT_FDS          = 256      # max file descriptors

# ── IMPROVEMENT #9: ZIP extraction limits ───────────────────
MAX_ZIP_TOTAL_SIZE_MB   = 100      # max total extracted size
MAX_ZIP_FILES           = 50       # max files in a ZIP

# ── IMPROVEMENT #10: Auto log cleanup ───────────────────────
LOG_RETENTION_DAYS      = 7        # delete logs older than this
CLEANUP_INTERVAL_HOURS  = 6        # run cleanup every N hours

# ── IMPROVEMENT #11: Circuit breaker for Telegram API ───────
CB_FAILURE_THRESHOLD    = 5        # failures before opening circuit
CB_RECOVERY_TIMEOUT     = 60       # seconds before trying again
cb_state                = {'failures': 0, 'last_failure': 0, 'open': False}

# ── IMPROVEMENT #12: Backup rotation on disk ────────────────
MAX_BACKUP_AGE_DAYS     = 30       # delete backups older than this

# ── IMPROVEMENT #13: TOTP 2FA ───────────────────────────────
TOTP_ISSUER             = 'DarkShadow'
totp_secrets            = {}       # user_id -> base32_secret
...
def is_upload_rate_limited(user_id):
    """✅ IMPROVEMENT: Check if user exceeded upload rate limit."""
    if user_id in admin_ids or user_id == OWNER_ID:
        return False
    now = time.time()
    with data_lock:
        upload_rate_data[user_id] = [
            t for t in upload_rate_data[user_id]
            if now - t < UPLOAD_RATE_WINDOW
        ]
        if len(upload_rate_data[user_id]) >= UPLOAD_RATE_LIMIT:
            return True
        upload_rate_data[user_id].append(now)
    return False
...
# Fix #7: Old notify_crash removed — replaced by enhanced version at line ~6533
# The watchdog calls below will use the new version when they actually execute
...
def format_size(size_bytes):
    # Fix #4: handle None and negative values
...
# Init — REMOVED: moved to main() to avoid double initialization (Fix #1)
# init_db()
# load_data()


# ============================================================
# 🛡️ IMPROVEMENT #14: GRACEFUL SHUTDOWN
# ============================================================

_shutdown_event = threading.Event()
_shutdown_done = False   # Fix #3: prevent triple shutdown

def graceful_shutdown(signum=None, frame=None):
    """Handle SIGTERM/SIGINT — kill all child scripts, close DB, exit clean."""
    global _shutdown_done
    if _shutdown_done:    # Fix #3: only run once
        return
    _shutdown_done = True

    sig_name = signal.Signals(signum).name if signum else "MANUAL"
    logger.info(f"🛑 Graceful shutdown triggered ({sig_name})...")
    _shutdown_event.set()
    ...
    for sk in list(bot_scripts.keys()):
        try:
            ...
            logger.error(f"Shutdown cleanup error {sk}: {e}")
    ...
    logger.info("✅ Dark Shadow shutdown complete.")
    sys.exit(0)

signal.signal(signal.SIGTERM, graceful_shutdown)
signal.signal(signal.SIGINT, graceful_shutdown)
# Fix #3: removed atexit.register — signal handlers already cover SIGTERM/SIGINT


# ============================================================
# 🛡️ IMPROVEMENT #15: PROCESS RESOURCE LIMITS
# ============================================================

def set_child_resource_limits():
    """
    Set resource limits for child processes (scripts).
    Prevents a single script from eating all RAM/CPU/fds.
    Call this in subprocess.Popen preexec_fn.
    """
    try:
        # Memory limit (RLIMIT_AS — address space)
        mem_bytes = MAX_SCRIPT_MEMORY_MB * 1024 * 1024
        resource.setrlimit(resource.RLIMIT_AS, (mem_bytes, mem_bytes))
        # CPU time limit
        resource.setrlimit(resource.RLIMIT_CPU, (MAX_SCRIPT_CPU_SECS, MAX_SCRIPT_CPU_SECS))
        # File descriptor limit
        resource.setrlimit(resource.RLIMIT_NOFILE, (MAX_SCRIPT_FDS, MAX_SCRIPT_FDS))
        # No core dumps
        resource.setrlimit(resource.RLIMIT_CORE, (0, 0))
    except Exception:
        pass  # Some systems may not support all limits
...
def run_script_generic(script_path, script_owner_id, user_folder, file_name, message_obj,
    ...
    try:
        ...
        # ✅ IMPROVEMENT #16: Resource limits on child process
        try:
            ...
            # Fix #8: Close log file handle if Popen fails
            try:
                log_file.close()
            except Exception:
                pass
...
def handle_text(message):
    ...
    # ✅ FIX: Skip command messages — let command handlers process them
...
def handle_document(message):
    ...
    # ✅ IMPROVEMENT #21: Upload rate limiting
    if is_upload_rate_limited(user_id):
        bot.reply_to(message, f"⚠️ <b>Upload rate limit!</b> Max {UPLOAD_RATE_LIMIT} uploads per {UPLOAD_RATE_WINDOW}s. Wait a moment.")
        return
        ...
        if file_ext == 'zip':
            ...
            try:
                with zipfile.ZipFile(tmp_path, 'r') as zf:
                    ...
                    # ✅ IMPROVEMENT #19: ZIP file count limit
                    if len(zf.namelist()) > MAX_ZIP_FILES:
                        raise Exception(f"Too many files in ZIP ({len(zf.namelist())}) > max {MAX_ZIP_FILES}")
                    # ✅ IMPROVEMENT #20: ZIP total extracted size limit
                    total_zip_size = sum(m.file_size for m in zf.infolist())
                    if total_zip_size > MAX_ZIP_TOTAL_SIZE_MB * 1024 * 1024:
                        raise Exception(f"ZIP total size ({format_size(total_zip_size)}) exceeds {MAX_ZIP_TOTAL_SIZE_MB}MB limit")
                    ...
                    if now >= run_at:
                        ...
                        if repeat == 'daily':
                            ...
                            conn2.execute('UPDATE scheduled_jobs SET active=0 WHERE id=?',
                                         (job['id'],))
                        conn2.commit()
                        # Fix #9: don't close thread-local connection in scheduler loop
                except Exception as e:
...
def check_config_health():
    ...
    if not os.environ.get('DASHBOARD_PASS'):
        issues.append("⚠️ DASHBOARD_PASS not set — using auto-generated password!")
    if issues:
...
def check_cooldown(script_key):
    """Fix #5: Properly handle first call — use None sentinel instead of 0."""
    now = time.time()
    last = script_cooldown.get(script_key)
    if last is None:
        # First call — record timestamp and allow
        script_cooldown[script_key] = now
        return 0
    elapsed = now - last
...
# Fix #2: removed — render_ping_worker starts in main() instead
...
# Fix #7: Renamed from notify_crash_v2 — this IS notify_crash now
...
# Patch notify_crash to use v2 version — Fix #7: already renamed above
# notify_crash = notify_crash_v2  ← REMOVED (no longer needed)
...
# Fix #10: Removed duplicate /ping handler — already handled by status_cmd (line 7432)
...
def start_command_v3(message):
    """
    🖤 Dark Shadow — Full Feature Menu System
    Shows ALL commands & features via categorized inline buttons.
    """
    uid = message.from_user.id
    ...
    try:
        ...
        if len(parts) > 1 and parts[1].startswith('ref_'):
            threading.Thread(target=process_referral, args=(uid, parts[1][4:]), daemon=True).start()
    except Exception as _e:
        logger.debug(f"start referral error: {_e}")

    # ── Register + guard checks ──────────────────────────────
    try:
        register_user(uid, message.from_user.first_name or 'User', message.from_user.username or '')
    ...
    is_admin = uid == OWNER_ID or uid in admin_ids
    my_running = len([k for k, v in bot_scripts.items() if v.get('user_id') == uid and is_bot_running_check(k)])

    welcome = (
        f"🖤 <b>Dark Shadow — Bot Hosting Platform</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 <b>{first_name}</b>  |  🆔 <code>{uid}</code>  |  {tier_label}\n"
        f"📦 Files: <b>{file_count}/{file_limit_str}</b>  |  🤖 Running: <b>{my_running}</b>\n"
        f"⏱️ Uptime: {get_uptime()}\n\n"
        f"📌 <b>Upload .py/.js → Runs 24/7 → Auto-restart on crash</b>\n\n"
        f"🔽 <b>Tap any category to explore ALL features:</b>"
    )

    # ── Main Menu Inline Keyboard ────────────────────────────
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("📂 File Management", callback_data="menu_files"),
        types.InlineKeyboardButton("🤖 Script Control", callback_data="menu_scripts"))
    markup.add(
        types.InlineKeyboardButton("🚀 Quick Deploy", callback_data="menu_deploy"),
        types.InlineKeyboardButton("⚙️ Config & Env", callback_data="menu_config"))
    markup.add(
        types.InlineKeyboardButton("🔍 Analysis & Health", callback_data="menu_health"),
        types.InlineKeyboardButton("📊 Stats & Analytics", callback_data="menu_stats"))
    markup.add(
        types.InlineKeyboardButton("⏰ Schedule & Cron", callback_data="menu_schedule"),
        types.InlineKeyboardButton("🎯 Bot Templates", callback_data="menu_templates"))
    markup.add(
        types.InlineKeyboardButton("💾 Backup & Restore", callback_data="menu_backup"),
        types.InlineKeyboardButton("💰 Subscription", callback_data="menu_subscribe"))
    markup.add(
        types.InlineKeyboardButton("👥 Referral & Trial", callback_data="menu_referral"),
        types.InlineKeyboardButton("🛠️ Settings", callback_data="menu_settings"))
    if is_admin:
        markup.add(
            types.InlineKeyboardButton("👑 Admin Panel", callback_data="menu_admin"),
            types.InlineKeyboardButton("📈 Admin Analytics", callback_data="menu_admin_analytics"))
    markup.add(
        types.InlineKeyboardButton("📋 All Commands List", callback_data="menu_allcmds"),
        types.InlineKeyboardButton("📞 Contact Owner", callback_data="menu_contact"))

    # ── Send with logo ───────────────────────────────────────
    sent = False

    # Try 1: Local logo file
    if os.path.exists(DARK_SCRIPT_LOGO_PATH):
        try:
            with open(DARK_SCRIPT_LOGO_PATH, 'rb') as _logo:
                bot.send_photo(message.chat.id, _logo, caption=welcome, parse_mode='HTML', reply_markup=markup)
            sent = True
            logger.info(f"start: sent with local logo (uid={uid})")
        except Exception as _e:
            logger.warning(f"start: local logo failed (uid={uid}): {_e}")

    # Try 2: Text message with markup
    if not sent:
        try:
            bot.send_message(message.chat.id, welcome, parse_mode='HTML', reply_markup=markup)
            sent = True
            logger.info(f"start: sent text with markup (uid={uid})")
        except Exception as _e:
            logger.warning(f"start: text+markup failed (uid={uid}): {_e}")

    # Try 3: Text without markup (in case markup is broken)
    if not sent:
        try:
            bot.send_message(message.chat.id, welcome, parse_mode='HTML')
            sent = True
            logger.info(f"start: sent text without markup (uid={uid})")
        except Exception as _e:
            logger.warning(f"start: text-only failed (uid={uid}): {_e}")

    # Try 4: Plain text, no HTML, no markup (last resort)
    if not sent:
        try:
            plain = (
                "Dark Shadow - Bot Hosting Platform\n"
                "===============================\n"
                f"User: {first_name} | ID: {uid}\n"
                f"Files: {file_count}/{file_limit_str}\n\n"
                "Upload .py/.js to get started!\n"
                f"Owner: {YOUR_USERNAME}"
            )
            bot.send_message(message.chat.id, plain)
            logger.info(f"start: sent plain text (uid={uid})")
        except Exception as _e:
...
# ============================================================
# 📋 MENU SYSTEM — Category callback handlers
# ============================================================

def _send_menu_page(chat_id, uid, title, text, extra_markup=None, edit_msg_id=None):
    """Helper to send or edit a menu page with back button."""
    markup = types.InlineKeyboardMarkup(row_width=2)
    if extra_markup:
        for btn in extra_markup:
            if isinstance(btn, list):
                markup.add(*btn)
            else:
                markup.add(btn)
    markup.add(types.InlineKeyboardButton("🔙 Back to Main Menu", callback_data="menu_main"))
    full_text = f"🖤 <b>Dark Shadow — {title}</b>\n{'━' * 35}\n\n{text}"

    # Try edit first
    if edit_msg_id:
        try:
            bot.edit_message_text(full_text, chat_id, edit_msg_id, parse_mode='HTML', reply_markup=markup)
            return
        except Exception as _e:
            logger.debug(f"menu edit failed: {_e}")

    # Try send with markup
    try:
        bot.send_message(chat_id, full_text, parse_mode='HTML', reply_markup=markup)
        return
    except Exception as _e:
        logger.warning(f"menu send failed: {_e}")

    # Try send without markup
    try:
        bot.send_message(chat_id, full_text, parse_mode='HTML')
    except Exception as _e:
        logger.error(f"menu send (no markup) failed: {_e}")
        # Last resort: plain text
        try:
            bot.send_message(chat_id, f"Dark Shadow - {title}\n\n{text}")
        except Exception:
            pass
...
def handle_menu_callback(call):
    ...
    if data == "menu_main":
        # Rebuild /start
        first_name = (call.from_user.first_name or 'User')[:20]
        tier_label = "👤 Free"
        file_limit = FREE_USER_LIMIT
        file_count = 0
        try: tier_label = get_user_status_label(uid)
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("📞 Contact Owner", callback_data="menu_contact"))
        try:
            bot.edit_message_text(welcome, chat_id, msg_id, parse_mode='HTML', reply_markup=markup)
        except Exception:
            pass
        return
    ...
    if data == "menu_files":
        text = (
            "<b>📂 FILE MANAGEMENT — All Commands</b>\n\n"
            "📤 <code>/upload</code> — Upload a new file (.py/.js/.zip/.json/.txt/.env/.yml)\n"
            "📂 <code>/files</code> — View all your uploaded files\n"
            "🔎 <code>/search term</code> — Search files by name\n"
            "👁️ <code>/preview</code> — Preview file content (read-only)\n"
            "✏️ <code>/rename</code> — Rename a file\n"
            "📋 <code>/duplicate</code> — Copy/duplicate a file\n"
            "🗑️ <code>/bulkdelete</code> — Delete multiple files at once\n"
            "🔀 <code>/sort</code> — Set file sort order (name/date/size/status)\n"
            "📁 <code>/folders</code> — Create & manage folders\n"
            "📊 <code>/diff</code> — Compare current file vs backup\n\n"
            "💡 <b>Tip:</b> Just send a file directly to upload it!\n"
            "Supported: <code>.py .js .zip .json .txt .env .yml .yaml</code>\n"
            f"Max size: <b>{MAX_FILE_SIZE_MB}MB</b>"
        )
        _send_menu_page(chat_id, uid, "File Management", text, edit_msg_id=msg_id)
        return

    # ── SCRIPT CONTROL ───────────────────────────────────────
    if data == "menu_scripts":
        text = (
            "<b>🤖 SCRIPT CONTROL — All Commands</b>\n\n"
            "▶️ <code>/r filename.py</code> — Quick run a script\n"
            "▶️ <code>/run</code> — Run a script (select from list)\n"
            "🛑 <code>/s filename.py</code> — Quick stop a script\n"
            "🛑 <code>/stop</code> — Stop a running script\n"
            "🔄 <code>/restart</code> — Restart a script\n"
            "🚀 <code>/multirun</code> — Run ALL your scripts at once\n"
            "🛑 <code>/killall</code> — Force stop ALL your scripts\n"
            "📋 <code>/logs</code> — View script logs (last output)\n"
            "📡 <code>/livelog</code> — Stream live logs (real-time)\n"
            "📤 <code>/outputalert</code> — Toggle output snippet alerts\n"
            "🔄 <code>/autorestart</code> — Toggle auto-restart per script\n"
            "📡 <code>/ports</code> — View port assignments (Flask/FastAPI)\n\n"
            "⚡ <b>Runtime:</b>\n"
            "• Scripts run <b>forever</b> — no time limit\n"
            "• Crash → auto-restarts immediately\n"
            "• Resource limits: 512MB RAM, 300s CPU per script\n"
            "• <code>/r</code> = quick run, <code>/s</code> = quick stop"
        )
        _send_menu_page(chat_id, uid, "Script Control", text, edit_msg_id=msg_id)
        return

    # ── QUICK DEPLOY ─────────────────────────────────────────
    if data == "menu_deploy":
        text = (
            "<b>🚀 QUICK DEPLOY — All Commands</b>\n\n"
            "🐙 <code>/gitdeploy URL</code> — Deploy from GitHub raw URL\n"
            "   Example: <code>/gitdeploy https://raw.githubusercontent.com/user/repo/main/bot.py</code>\n\n"
            "🔄 <code>/gitsync file.py URL</code> — Auto-sync from GitHub every hour\n"
            "🐙 <code>/githubsetup</code> — Setup GitHub webhook (auto-deploy on push)\n"
            "🐙 <code>/githubstatus</code> — Check GitHub sync/webhook status\n\n"
            "🚀 <code>/deploy</code> — One-click deploy with pre-flight checks\n"
            "📦 <code>/runproject</code> — Detect & run main file from project\n\n"
            "📋 <code>/installreqs</code> — Install from requirements.txt\n"
            "🟨 <code>/npminit</code> — Run npm install from package.json\n\n"
            "💡 <b>Tip:</b> Upload a ZIP → auto-extracted → run any file inside!"
        )
        _send_menu_page(chat_id, uid, "Quick Deploy", text, edit_msg_id=msg_id)
        return

    # ── CONFIG & ENV ─────────────────────────────────────────
    if data == "menu_config":
        text = (
            "<b>⚙️ CONFIG & ENV VARS — All Commands</b>\n\n"
            "🔧 <code>/setenv</code> — Upload a .env file for your scripts\n"
            "📝 <code>/enveditor</code> — Visual .env file editor (add/edit/delete vars)\n\n"
            "⚙️ <code>/setvar file.py KEY VALUE</code> — Set env var for specific script\n"
            "📋 <code>/listvars</code> — List all your env vars\n"
            "🗑️ <code>/delvar file.py KEY</code> — Delete an env var\n\n"
            "🔍 <code>/configcheck</code> — Check bot config health\n\n"
            "💡 <b>How env vars work:</b>\n"
            "• .env file vars are auto-injected when running scripts\n"
            "• Per-script vars (/setvar) override .env values\n"
            "• Format: <code>BOT_TOKEN=your_token_here</code>"
        )
        _send_menu_page(chat_id, uid, "Config & Env", text, edit_msg_id=msg_id)
        return

    # ── ANALYSIS & HEALTH ────────────────────────────────────
    if data == "menu_health":
        text = (
            "<b>🔍 ANALYSIS & HEALTH — All Commands</b>\n\n"
            "🏥 <code>/health</code> — Script health score (0-100)\n"
            "🔍 <code>/diagnose</code> — Auto-detect errors & suggest fixes\n"
            "🛡️ <code>/scan</code> — Security malware scan (dangerous patterns)\n"
            "🔍 <code>/configcheck</code> — Bot config health check\n\n"
            "🖥️ <code>/resources</code> — System resources (CPU/RAM/Disk/Network)\n"
            "⚡ <code>/speed</code> — Bot latency & performance test\n"
            "🛠️ <code>/debug</code> — Toggle debug mode (verbose logging)\n\n"
            "💡 <b>Health Score factors:</b>\n"
            "• Syntax check (+25) • File exists (+20)\n"
            "• No placeholder token (+15) • Env vars set (+10)\n"
            "• Auto-restart ON (+10)"
        )
        _send_menu_page(chat_id, uid, "Analysis & Health", text, edit_msg_id=msg_id)
        return

    # ── STATS & ANALYTICS ────────────────────────────────────
    if data == "menu_stats":
        text = (
            "<b>📊 STATS & ANALYTICS — All Commands</b>\n\n"
            "📊 <code>/stats</code> — System stats (CPU/RAM/Uptime/Bots)\n"
            "📊 <code>/myusage</code> — Your personal usage stats\n"
            "📱 <code>/status</code> — Your subscription & running scripts\n"
            "🏆 <code>/leaderboard</code> — Top users by runs & uploads\n"
            "📈 <code>/graph</code> — Activity graph (hourly/daily)\n\n"
            "📈 <code>/weeklystats</code> — Weekly stats (admin)\n"
            "📅 <code>/monthlystats</code> — Monthly stats (admin)\n"
            "📊 <code>/allusage</code> — All users usage (admin)\n"
            "👤 <code>/useractivity USER_ID</code> — Per-user activity log (admin)\n"
            "🤖 <code>/jobs</code> — Background jobs status\n"
            "📡 <code>/resources</code> — Detailed system resources"
        )
        _send_menu_page(chat_id, uid, "Stats & Analytics", text, edit_msg_id=msg_id)
        return

    # ── SCHEDULE & CRON ──────────────────────────────────────
    if data == "menu_schedule":
        text = (
            "<b>⏰ SCHEDULE & CRON — All Commands</b>\n\n"
            "⏰ <code>/schedule</code> — Schedule a script to run later\n"
            "📋 <code>/myschedules</code> — View & manage your scheduled jobs\n"
            "⏰ <code>/cronbuilder</code> — Visual cron job builder\n\n"
            "⏰ <b>Schedule options:</b>\n"
            "• 1 min / 1 hour / 1 day / 1 week\n"
            "• Daily at specific time\n"
            "• Weekly\n"
            "• Custom: <code>YYYY-MM-DD HH:MM daily</code>\n\n"
            "💡 <b>Jobs run automatically at scheduled time!</b>\n"
            "Scripts auto-start even if bot restarts."
        )
        _send_menu_page(chat_id, uid, "Schedule & Cron", text, edit_msg_id=msg_id)
        return

    # ── BOT TEMPLATES ────────────────────────────────────────
    if data == "menu_templates":
        text = (
            "<b>🎯 BOT TEMPLATES — Ready-Made Bots</b>\n\n"
            "🎯 <code>/templates</code> — View all templates\n\n"
            "<b>Available Templates:</b>\n"
            "🔁 <b>Echo Bot</b> — Jo bhejo wahi wapas aata hai\n"
            "👋 <b>Welcome Bot</b> — Naye members ko welcome karta hai\n"
            "🔢 <b>Calculator Bot</b> — Basic math calculator\n"
            "📊 <b>Poll Bot</b> — Group mein polls banata hai\n"
            "⏰ <b>Reminder Bot</b> — Reminders set karna\n\n"
            "💡 <b>One-click install!</b> Template apne folder mein save ho jaata hai.\n"
            "Sirf BOT_TOKEN set karo aur Run karo! 🚀"
        )
        _send_menu_page(chat_id, uid, "Bot Templates", text, edit_msg_id=msg_id)
        return

    # ── BACKUP & RESTORE ─────────────────────────────────────
    if data == "menu_backup":
        text = (
            "<b>💾 BACKUP & RESTORE — All Commands</b>\n\n"
            "💾 <code>/backup</code> — Backup your files (one or all)\n"
            "📋 <code>/mybackups</code> — View & restore backups\n"
            "📊 <code>/diff</code> — Compare current file vs backup\n\n"
            "💾 <b>Backup features:</b>\n"
            "• Auto-backup before every file edit\n"
            "• Max 5 backups per file kept\n"
            "• Backups older than 30 days auto-deleted\n"
            "• One-click restore from any backup\n\n"
            "💡 <b>Tip:</b> Always backup before major changes!"
        )
        _send_menu_page(chat_id, uid, "Backup & Restore", text, edit_msg_id=msg_id)
        return
    ...
    if data == "menu_subscribe":
        text = (
            ...
            "💰 <code>/buy</code> — View & buy plans\n"
            "💎 <code>/changeplan</code> — Upgrade or downgrade\n"
            "₿ <code>/cryptopay</code> — Pay with crypto (USDT/BTC)\n"
            "📱 <code>/status</code> — Check your subscription\n\n"
            f"👤 <b>Free:</b> {FREE_USER_LIMIT} files\n"
            "⭐ <b>Basic:</b> 100 PKR/mo — 15 files\n"
            "💎 <b>Pro:</b> 250 PKR/mo — 30 files\n"
            "👑 <b>Premium:</b> 500 PKR/mo — 999 files\n\n"
            "💡 <b>All plans include:</b>\n"
            "• 24/7 script hosting\n"
            "• Auto-restart on crash\n"
            "• Unlimited runtime\n"
            "• Log access & live streaming"
        )
        markup = [
            [types.InlineKeyboardButton("💰 Buy Plan Now", callback_data="buy_basic")]
        ]
        _send_menu_page(chat_id, uid, "Subscription", text, extra_markup=markup, edit_msg_id=msg_id)
        return
    ...
    if data == "menu_referral":
        ...
        text = (
            "<b>👥 REFERRAL & TRIAL</b>\n\n"
            "🎁 <code>/trial</code> — Get 3-day free trial (one-time)\n"
            "👥 <code>/refer</code> — Your referral link & stats\n\n"
            f"🔗 <b>Your Referral Code:</b> <code>{code}</code>\n"
            f"🔗 <b>Your Link:</b> <code>https://t.me/YourBot?start=ref_{code}</code>\n\n"
            "👥 <b>How referrals work:</b>\n"
            "• Share your link with friends\n"
            "• When they join → BOTH get +3 days free!\n"
            "• No limit on referrals\n\n"
            "🎁 <b>Trial:</b> 3 days free, 5 file limit\n"
            "After trial → upgrade with /buy"
        )
        _send_menu_page(chat_id, uid, "Referral & Trial", text, edit_msg_id=msg_id)
        return
    ...
    if data == "menu_settings":
        text = (
            ...
            "✏️ <code>/edit</code> — Edit file content directly in chat\n"
            "📝 <code>/enveditor</code> — Visual .env file editor\n"
            "🔄 <code>/autorestart</code> — Toggle auto-restart per script\n"
            "📤 <code>/outputalert</code> — Toggle output alerts per script\n"
            "🔀 <code>/sort</code> — Set file sort order\n\n"
            "🔧 <code>/configcheck</code> — Check bot config health\n"
            "🛠️ <code>/debug</code> — Toggle debug mode\n\n"
            "🔐 <code>/setup2fa</code> — Setup TOTP 2FA (Google Authenticator)\n"
            "🔐 <code>/verify2fa</code> — Verify 2FA code\n\n"
            f"⚡ <b>Rate Limit:</b> {RATE_LIMIT_MESSAGES} msgs / {RATE_LIMIT_WINDOW}s\n"
            f"📦 <b>Max File Size:</b> {MAX_FILE_SIZE_MB}MB\n"
            f"🗑️ <b>Log Retention:</b> {LOG_RETENTION_DAYS} days"
        )
        _send_menu_page(chat_id, uid, "Settings", text, edit_msg_id=msg_id)
        return
    ...
    if data == "menu_admin":
        if not is_admin:
            ...
            return
        text = (
            "<b>👑 ADMIN PANEL — All Commands</b>\n\n"
            "<b>👤 User Management:</b>\n"
            "🚫 <code>/ban USER_ID [reason]</code> — Ban a user\n"
            "✅ <code>/unban USER_ID</code> — Unban a user\n"
            "⚠️ <code>/warn USER_ID [reason]</code> — Warn user (auto-ban after 3)\n"
            "🔍 <code>/userinfo USER_ID</code> — Detailed user info\n"
            "👑 <code>/addadmin USER_ID</code> — Add admin\n"
            "👑 <code>/removeadmin USER_ID</code> — Remove admin\n\n"
            "<b>📢 Communication:</b>\n"
            "📢 <code>/broadcast</code> — Message all users\n"
            "🔔 <code>/alerts</code> — Toggle notification types\n"
            "🔧 <code>/maintenance</code> — Toggle maintenance mode\n"
            "🔒 <code>/lock</code> — Lock/unlock bot\n\n"
            "<b>💰 Payments:</b>\n"
            "💳 <code>/subscribe USER_ID DAYS</code> — Add subscription\n"
            "✅ <code>/approve USER_ID</code> — Approve payment\n"
            "❌ <code>/reject USER_ID</code> — Reject payment\n"
            "💰 <code>/revenue</code> — Revenue dashboard\n"
            "📊 <code>/exportrevenue</code> — Export revenue CSV\n\n"
            "<b>🔐 Security:</b>\n"
            "🔐 <code>/setpin</code> — Set admin 2FA PIN\n"
            "🌐 <code>/addip IP</code> — IP whitelist for dashboard\n"
            "🌐 <code>/removeip IP</code> — Remove IP from whitelist\n"
            "📋 <code>/auditlog</code> — View command audit log\n"
            "🛑 <code>/stopall</code> — Stop ALL running bots"
        )
        _send_menu_page(chat_id, uid, "Admin Panel", text, edit_msg_id=msg_id)
        return

    # ── ADMIN ANALYTICS ──────────────────────────────────────
    if data == "menu_admin_analytics":
        if not is_admin:
            bot.answer_callback_query(call.id, "❌ Admin only!")
            return
        text = (
            "<b>📈 ADMIN ANALYTICS — All Commands</b>\n\n"
            "📊 <code>/analytics</code> — Today's analytics report\n"
            "📈 <code>/weeklystats</code> — 7-day stats with graph\n"
            "📅 <code>/monthlystats</code> — Monthly overview\n"
            "📊 <code>/allusage</code> — All users usage (top 10)\n"
            "👤 <code>/useractivity USER_ID</code> — Per-user activity\n"
            "🤖 <code>/jobs</code> — Background jobs status\n"
            "🖥️ <code>/resources</code> — CPU/RAM/Disk/Network\n"
            "⚡ <code>/setalert cpu 85</code> — Set resource alert threshold\n\n"
            "📊 <b>Daily Report:</b> Auto-sent at configured time\n"
            "🔥 <b>Resource Alerts:</b> Auto-sent when CPU/RAM > 85%"
        )
        _send_menu_page(chat_id, uid, "Admin Analytics", text, edit_msg_id=msg_id)
        return

    # ── ALL COMMANDS LIST ─────────────────────────────────────
    if data == "menu_allcmds":
        running = len([k for k in bot_scripts if is_bot_running_check(k)])
        text = (
            "<b>📋 ALL COMMANDS — Complete List</b>\n\n"
            "<b>📂 Files:</b>\n"
            "  /upload /files /rename /duplicate /bulkdelete\n"
            "  /search /preview /diff /sort /folders\n\n"
            "<b>🤖 Scripts:</b>\n"
            "  /r /s /run /stop /restart /multirun /killall\n"
            "  /logs /livelog /outputalert /autorestart /ports\n\n"
            "<b>🚀 Deploy:</b>\n"
            "  /gitdeploy /gitsync /githubsetup /githubstatus\n"
            "  /deploy /runproject /installreqs /npminit\n\n"
            "<b>⚙️ Config:</b>\n"
            "  /setenv /enveditor /setvar /listvars /delvar\n\n"
            "<b>🔍 Health:</b>\n"
            "  /health /diagnose /scan /configcheck /resources /speed\n\n"
            "<b>📊 Stats:</b>\n"
            "  /stats /myusage /status /leaderboard /graph\n"
            "  /weeklystats /monthlystats /allusage /useractivity /jobs\n\n"
            "<b>⏰ Schedule:</b>\n"
            "  /schedule /myschedules /cronbuilder\n\n"
            "<b>💾 Backup:</b>\n"
            "  /backup /mybackups\n\n"
            "<b>💰 Money:</b>\n"
            "  /buy /trial /refer /changeplan /cryptopay\n\n"
            "<b>🎯 Templates:</b>\n"
            "  /templates\n\n"
            "<b>🛠️ Tools:</b>\n"
            "  /edit /debug /setup2fa /verify2fa\n\n"
            "<b>📋 Info:</b>\n"
            "  /start /help /h\n\n"
            f"🤖 Running bots: <b>{running}</b>\n"
            f"👥 Total users: <b>{len(active_users)}</b>"
        )
        _send_menu_page(chat_id, uid, "All Commands", text, edit_msg_id=msg_id)
        return

    # ── SHOW PLANS (from expiry alerts) ──────────────────────
    if data == "show_plans":
        bot.answer_callback_query(call.id)
        text = (
            "╔══════════════════════════════════════╗\n"
    ...
    if data == "menu_contact":
        ...
        markup = [
            [types.InlineKeyboardButton("💬 Message Owner", url=f"https://t.me/{YOUR_USERNAME.replace('@','')}")]
        ]
        _send_menu_page(chat_id, uid, "Contact", text, extra_markup=markup, edit_msg_id=msg_id)
        return


# ============================================================
# 📋 /help — Now redirects to menu system
# ============================================================

@bot.message_handler(commands=['help', 'h'])
@security_check
def help_cmd(message):
    """Help command — shows the same full menu as /start."""
    uid = message.from_user.id
    audit_log(uid, 'help')
    track_command('help', uid)
    is_admin = uid == OWNER_ID or uid in admin_ids

    welcome = (
        "🖤 <b>Dark Shadow — Help Center</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🔽 <b>Select a category to see all commands:</b>"
    )
...
# ============================================================
# 🛡️ IMPROVEMENT #22: CIRCUIT BREAKER FOR TELEGRAM API
# ============================================================

def circuit_breaker_call(func, *args, **kwargs):
    """
    Wrap Telegram API calls with circuit breaker pattern.
    If too many failures, stop calling for CB_RECOVERY_TIMEOUT seconds.
    """
    if cb_state['open']:
        elapsed = time.time() - cb_state['last_failure']
        if elapsed < CB_RECOVERY_TIMEOUT:
            logger.warning(f"Circuit OPEN — skipping Telegram call (recovery in {CB_RECOVERY_TIMEOUT - elapsed:.0f}s)")
            return None
        else:
    ...
    try:
        ...
        if cb_state['failures'] >= CB_FAILURE_THRESHOLD:
            cb_state['open'] = True
            logger.error(f"Circuit OPENED after {cb_state['failures']} failures: {e}")
            try:
                notify_owner(f"🚨 <b>Circuit Breaker OPEN!</b>\nTelegram API failing ({cb_state['failures']} errors)\nLast: {str(e)[:100]}")
            except Exception:
                pass
        raise
...
# ============================================================
# 🛡️ IMPROVEMENT #23: AUTO LOG CLEANUP
# ============================================================
                ...
                for fname in os.listdir(LOGS_DIR):
                    ...
                    if os.path.isfile(fpath) and fname.endswith('.log'):
                        if os.path.getmtime(fpath) < cutoff:
                            ...
                            cleaned += 1
                if cleaned:
                    logger.info(f"🧹 Log cleanup: removed {cleaned} old log files")
            except Exception as e:
                logger.error(f"Log cleanup error: {e}")
            time.sleep(CLEANUP_INTERVAL_HOURS * 3600)
    Thread(target=_worker, daemon=True).start()
    logger.info(f"🧹 Log cleanup started (retention: {LOG_RETENTION_DAYS}d)")


# ============================================================
# 🛡️ IMPROVEMENT #24: BACKUP ROTATION ON DISK
# ============================================================

def start_backup_rotation_worker():
    """Delete backup files older than MAX_BACKUP_AGE_DAYS."""
    def _worker():
        while True:
            try:
                cutoff = time.time() - (MAX_BACKUP_AGE_DAYS * 86400)
                cleaned = 0
                for user_dir in os.listdir(BACKUP_DIR):
                    ...
                    if not os.listdir(user_path):
                        os.rmdir(user_path)
                if cleaned:
                    logger.info(f"🧹 Backup rotation: removed {cleaned} old backups")
                    # Also clean DB records
                    try:
                        conn = get_db()
                        cutoff_iso = (datetime.now() - timedelta(days=MAX_BACKUP_AGE_DAYS)).isoformat()
                        conn.execute('DELETE FROM backups WHERE created_at < ?', (cutoff_iso,))
                        conn.commit()
                    except Exception:
                        pass
            except Exception as e:
    ...
    Thread(target=_worker, daemon=True).start()
    logger.info(f"🧹 Backup rotation started (max age: {MAX_BACKUP_AGE_DAYS}d)")


# ============================================================
# 🛡️ IMPROVEMENT #25: TOTP 2FA (Google Authenticator)
# ============================================================

def _totp_generate_secret():
    """Generate a random base32 secret for TOTP."""
    return _b64.b32encode(os.urandom(20)).decode('utf-8').rstrip('=')
...
def _totp_get_code(secret):
    ...
    key = _b64.b32decode(padded)
    # Counter = 30-second intervals since epoch
    counter = int(_time_mod.time()) // 30
    # HMAC-SHA1
    counter_bytes = _struct.pack('>Q', counter)
    hmac_hash = hmac.new(key, counter_bytes, hashlib.sha1).digest()
    # Dynamic truncation
    offset = hmac_hash[-1] & 0x0f
    code = _struct.unpack('>I', hmac_hash[offset:offset+4])[0] & 0x7fffffff
...
def _totp_verify(secret, code, window=1):
    ...
    for offset in range(-window, window + 1):
        padded = secret + '=' * (8 - len(secret) % 8) if len(secret) % 8 else secret
        key = _b64.b32decode(padded)
        counter = int(_time_mod.time()) // 30 + offset
        counter_bytes = _struct.pack('>Q', counter)
        hmac_hash = hmac.new(key, counter_bytes, hashlib.sha1).digest()
        o = hmac_hash[-1] & 0x0f
        expected = _struct.unpack('>I', hmac_hash[o:o+4])[0] & 0x7fffffff
        if str(expected % 1000000).zfill(6) == code:
            return True
    return False

def _totp_get_uri(secret, user_id):
    """Generate otpauth:// URI for QR code generation."""
    return f"otpauth://totp/{TOTP_ISSUER}:{user_id}?secret={secret}&issuer={TOTP_ISSUER}&digits=6&period=30"

@bot.message_handler(commands=['setup2fa'])
def setup2fa_cmd(message):
    uid = message.from_user.id
    if uid != OWNER_ID and uid not in admin_ids:
        bot.reply_to(message, "❌ Admin only!")
        return
    secret = _totp_generate_secret()
    totp_secrets[uid] = secret
    uri = _totp_get_uri(secret, uid)
    # Save to DB
    try:
        conn = get_db()
    ...
    admin_pin_required.add(uid)
    bot.reply_to(message,
        f"🔐 <b>TOTP 2FA Setup</b>\n\n"
        f"1️⃣ Install Google Authenticator / Authy\n"
        f"2️⃣ Add this secret key:\n<code>{secret}</code>\n\n"
        f"Or scan QR (use link below in any QR generator):\n"
        f"<code>{uri}</code>\n\n"
        f"3️⃣ Verify with: <code>/verify2fa 123456</code>\n\n"
        f"⚠️ Save this secret somewhere safe!",
        parse_mode='HTML')

@bot.message_handler(commands=['verify2fa'])
def verify2fa_cmd(message):
    uid = message.from_user.id
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /verify2fa <6-digit-code>")
        return
    secret = totp_secrets.get(uid)
    if not secret:
        # Try loading from DB
        try:
            conn = get_db()
            row = conn.execute('SELECT pin_hash FROM admin_pins WHERE user_id=?', (uid,)).fetchone()
            if row:
                bot.reply_to(message, "❌ 2FA secret not loaded. Use /setup2fa to reconfigure.")
                return
        except Exception:
            pass
        bot.reply_to(message, "❌ 2FA not set up! Use /setup2fa first.")
        return
    code = parts[1].strip()
    if _totp_verify(secret, code):
        admin_pin_verified[uid] = datetime.now() + timedelta(minutes=30)
        bot.reply_to(message, "✅ <b>2FA Verified!</b> Access granted for 30 minutes.", parse_mode='HTML')
    else:
        bot.reply_to(message, "❌ Invalid code! Try again.")


# ============================================================
# 🛡️ IMPROVEMENT #26: METRICS ENDPOINT (Prometheus format)
# ============================================================
...
def metrics_endpoint():
    ...
    running = len([k for k in bot_scripts if is_bot_running_check(k)])
    metrics = [
        f'# HELP darkshadow_users_total Total registered users',
        f'# TYPE darkshadow_users_total gauge',
        f'darkshadow_users_total {len(active_users)}',
        f'# HELP darkshadow_bots_running Currently running bots',
        f'# TYPE darkshadow_bots_running gauge',
        f'darkshadow_bots_running {running}',
        f'# HELP darkshadow_cpu_percent CPU usage percentage',
        f'# TYPE darkshadow_cpu_percent gauge',
        f'darkshadow_cpu_percent {psutil.cpu_percent()}',
        f'# HELP darkshadow_ram_percent RAM usage percentage',
        f'# TYPE darkshadow_ram_percent gauge',
        f'darkshadow_ram_percent {psutil.virtual_memory().percent}',
        f'# HELP darkshadow_disk_percent Disk usage percentage',
        f'# TYPE darkshadow_disk_percent gauge',
        f'darkshadow_disk_percent {psutil.disk_usage("/").percent}',
        f'# HELP darkshadow_uptime_seconds Bot uptime in seconds',
        f'# TYPE darkshadow_uptime_seconds gauge',
        f'darkshadow_uptime_seconds {(datetime.now() - bot_start_time).total_seconds()}',
        f'# HELP darkshadow_circuit_breaker_open Whether Telegram circuit breaker is open',
        f'# TYPE darkshadow_circuit_breaker_open gauge',
        f'darkshadow_circuit_breaker_open {1 if cb_state["open"] else 0}',
    ]
    return '\n'.join(metrics), 200, {'Content-Type': 'text/plain; charset=utf-8'}


# ============================================================
# 🛡️ IMPROVEMENT #27: DB WAL CHECKPOINT (prevent infinite growth)
# ============================================================

def start_db_checkpoint_worker():
    """Periodically checkpoint SQLite WAL to prevent infinite growth."""
    def _worker():
        while True:
            try:
                conn = get_db()
                ...
                logger.error(f"DB checkpoint error: {e}")
            time.sleep(3600)  # Hourly
    Thread(target=_worker, daemon=True).start()
    logger.info("🛡️ DB WAL checkpoint worker started (hourly)")


# ============================================================
# 🛡️ IMPROVEMENT #28: DB CONNECTION CLEANUP ON THREAD EXIT
# ============================================================

_original_thread_init = threading.Thread.__init__

def _patched_thread_init(self, *args, **kwargs):
    """Patch Thread to clean up DB connections when thread exits."""
    ...
    def _run_with_cleanup():
        try:
            ...
            # Clean up thread-local DB connection
            if hasattr(_thread_local, 'conn') and _thread_local.conn is not None:
                try:
                    _thread_local.conn.close()
                except Exception:
                    pass
                _thread_local.conn = None
    self.run = _run_with_cleanup

# Only patch if not already patched
if not hasattr(threading.Thread, '_db_cleanup_patched'):
    threading.Thread.__init__ = _patched_thread_init
    threading.Thread._db_cleanup_patched = True


# ============================================================
# 🛡️ IMPROVEMENT #29: FLASK API RATE LIMITING
# ============================================================

flask_rate_limit = defaultdict(list)  # ip -> [timestamps]
FLASK_RATE_LIMIT = 60  # requests per minute
...
def rate_limit_api():
    ...
    if request.path.startswith('/api/'):
        ...
        now = time.time()
        flask_rate_limit[ip] = [t for t in flask_rate_limit[ip] if now - t < FLASK_RATE_WINDOW]
        if len(flask_rate_limit[ip]) >= FLASK_RATE_LIMIT:
            return jsonify({'error': 'Rate limit exceeded'}), 429
        flask_rate_limit[ip].append(now)


# ============================================================
# 🛡️ IMPROVEMENT #30: /start IMAGE BANNER + DASHBOARD LOGO
# ============================================================

DARK_SCRIPT_LOGO_PATH = os.path.join(BASE_DIR, 'dark_script_logo.jpeg')

def get_start_markup(uid):
    """Build the main keyboard for /start."""
    try:
        return get_main_keyboard(uid)
    except Exception:
        return None


# ============================================================
# 🛡️ IMPROVEMENT #31: READINESS / LIVENESS PROBES
# ============================================================

@app.route('/ready')
def readiness_probe():
    """Kubernetes readiness probe — is the bot ready to serve traffic?"""
    try:
        conn = get_db()
        conn.execute('SELECT 1').fetchone()
...
def liveness_probe():
    ...
    return jsonify({'alive': True, 'uptime': get_uptime()}), 200


# ============================================================
# 🛡️ IMPROVEMENT #32: GRACEFUL DB DEGRADATION
# ============================================================

_fallback_db = {}  # In-memory fallback when SQLite fails

def safe_db_execute(query, params=(), fetch=False):
    """
    Execute DB query with graceful degradation.
    If SQLite fails, falls back to in-memory storage.
    """
    try:
        conn = get_db()
        if fetch:
        ...
        logger.error(f"DB degraded — falling back to memory: {e}")
        # Log to fallback
        _fallback_db.setdefault('errors', []).append({
            'query': query[:100], 'error': str(e)[:100],
            'time': datetime.now().isoformat()
        })
        return False
...
# ============================================================
# 🛡️ IMPROVEMENT #33: SHUTDOWN /stop GRACEFUL ENDPOINT
# ============================================================

@app.route('/shutdown', methods=['POST'])
def web_shutdown():
    """Webhook to trigger graceful shutdown (protected)."""
    token = request.args.get('token', '')
    if token != app.secret_key[:16]:
        return jsonify({'error': 'Unauthorized'}), 401
    threading.Thread(target=lambda: graceful_shutdown(signal.SIGTERM, None), daemon=True).start()
...
def main():
    ...
    # ✅ NEW IMPROVEMENTS
    start_log_cleanup_worker()        # 🧹 Auto-delete old logs
    start_backup_rotation_worker()    # 🧹 Rotate old backups
    start_db_checkpoint_worker()      # 🛡️ WAL checkpoint hourly
